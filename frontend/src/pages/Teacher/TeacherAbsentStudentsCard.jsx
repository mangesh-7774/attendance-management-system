  ...
  const fetchAttendance = async () => {
    try {
      ...
      if (response.data.success) {
        const absentRecords = response.data.records.filter(
    ...
    <div className="pt-24 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">
          Complete list of students marked absent today
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-md overflow-hidden">
        {loading ? (
          ...
          <p className="p-6 text-gray-500">No absent students today 🎉</p>
        ) : (
          <table className="w-full text-left">
            {/* Table Head */}
            <thead className="bg-gray-100 text-gray-600 text-sm uppercase">
              <tr>
                <th className="px-6 py-4">Rll No</th>
              ...
              {records.map((record) => (
                ...
                >
                  <td className="px-6 py-4">
                    {record.student?.studentData?.rollNumber}
                  </td>
                  <td className="px-6 py-4">
...
export default TeacherAbsentStudentsCard;
